class Campaign::SummaryThirty
  ...
  field :cui,  as: :currency_id, type: String
  ...
  index({ company_id: 1,content_provider_id: 1,campaign_id: 1,date:1,active:1 })
  ...
  def self.update_active(c_id,date)
    where(:d => {
        "$lt" => date},:cid=> c_id).update_all(:a => false)
  end
  ...
  def self.active(cy_id,date,sort)
    ...
    if cy_id=="1"
      results=where(:a=>true,
                     :d => {
                         "$lte" => date}).
          only(["campaign_name","service_code","live_operator_count","live_media_count","acquisition_model", "valid_engagments",
                "valid_subscriptions", "active_subscribers","un_subscriptions","valid_sub_percent",
      ...
      results=where( :cyid => cy_id, :a=>true,
             :d => {
                 "$lte" => date}).
          only(["campaign_name","service_code","live_operator_count","live_media_count","acquisition_model", "valid_engagments",
                "valid_subscriptions", "active_subscribers","un_subscriptions","valid_sub_percent",
  ...
  def self.find_sub_by_cid_and_date(cy_id,c_id,date)
    where( :cyid => cy_id,:cid => c_id,:d => {
