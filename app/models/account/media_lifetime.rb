class Account::MediaLifetime
  ...
  index(company_id:1,campaign_id: 1, media_id: 1, date: 1,acitve:1 )

  def self.delete_existing(cy_id,me_id,date)
    where(:cyid => cy_id, :meid => me_id , :d => date).delete
  end

  def self.update_active(cy_id,media_id,date)
    where(:d => {
        "$lte" => date},:cyid => cy_id,:meid=> media_id).update_all(:a => false)
  end
