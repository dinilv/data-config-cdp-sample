class Aggregator::CampaignDailyJob < ApplicationJob

  @queue = :aggregator_campaign_daily


  def self.perform(campaign_id,date)

    @@logger_aggregator.info("In Campaign Daily Queue Jobs:" + campaign_id.to_s)
    puts "Campaign Daily job"
    puts campaign_id,date

    begin
      aggregate_campaign_daily(campaign_id,date)
    rescue => e
      exception_job = {:action => "aggreagtor_campaign_daily_job", :exception => e.message , :backtrace => e.backtrace.inspect, :version=>"v1"}
  ...
  def self.aggregate_campaign_daily(campaign_id,date_string)
    ...
    campaign_date_filter = {MATCH_AGG_OP =>
                                { CAMPAIGN_ID_SHORT => campaign_id,
                                  ...
                                  COMPANY_ID_SHORT => ADMIN_ACCOUNT
                                }
    }
    ...
    if standard_campaign_report.count()>0
      standard_campaign_report.each do |each_data|
        ...
        #details
        @@config_service.add_admin_campaign_details(campaign_summary_daily_hash)
        ...
        Campaign::SummaryDaily.delete_existing(ADMIN_ACCOUNT,campaign_id, formatted_date)
